...
class AnalyticalDatabase:
    ...
    def load_dataframe(self, df: pd.DataFrame, table_name: str) -> TableSchema:
        """Register a DataFrame as a table, replacing any existing table of
        the same name. Numeric-looking string columns (e.g. a CSV where
        "revenue" was exported as text, possibly with thousands
        separators like "1,234.56") are coerced to a real numeric dtype
        first -- see _coerce_numeric_like_columns() docstring for why this
        matters and its documented tradeoff."""
...
_NUMERIC_COERCION_THRESHOLD = 0.9  # a column must be at least this numeric-parseable to be coerced


_CURRENCY_AND_THOUSANDS_PATTERN = re.compile(r"[,$€£¥₹]")


def _clean_numeric_string_series(series: pd.Series) -> pd.Series:
    """Strip thousands-separator commas, common currency symbols, and
    surrounding whitespace from a string series, in preparation for
    pd.to_numeric(). Used identically by both the detection pass (does
    this column qualify for coercion?) and the actual coercion, so the two
    can never silently disagree about what counts as 'numeric-looking'."""
    return series.str.strip().str.replace(_CURRENCY_AND_THOUSANDS_PATTERN, "", regex=True)


def _coerce_numeric_like_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Coerce string/object columns that are overwhelmingly numeric (after
    stripping thousands-separator commas, common currency symbols, and
    surrounding whitespace) to a real numeric dtype, BEFORE the data is
    written to SQL.

    WHY THIS MATTERS: every numeric-column check in this app (mock LLM
    metric resolution, the SQL validator's aggregation checks, chart
    building) inspects the column's actual SQL type. A revenue column
    that's numeric in spirit but was exported as text in the source CSV
    (common with Excel exports, or values formatted like "1,234.56" or
    "$1,234.56") would otherwise be written to SQLite as TEXT and become
    invisible to every one of those checks -- "what is the total revenue?"
    would fail to find a metric column at all, even though a human looking
    at the data would immediately recognize it as numeric.

    KNOWN TRADEOFF: a column of purely-numeric-looking identifiers (e.g.
    a zip code or an all-digit order ID with no letters) will also be
    coerced to numeric and could then be mistakenly offered as a
    fallback "the only numeric column" metric candidate. This was judged
    the safer default -- the alternative (never coercing) makes a much
    more common real-world case (a formatted numeric metric) silently
    unusable. The 90% threshold means a column needs to be OVERWHELMINGLY
    numeric-looking, not just partially, to be affected. Percent signs
    ("5%") are deliberately NOT stripped -- coercing "%"-suffixed values
    would silently change their scale/meaning, which is a materially
    different (and riskier) transformation than removing a thousands
    ...
    for col in df.columns:
        ...
        if len(non_null) == 0:
            continue
        cleaned = _clean_numeric_string_series(non_null)
        parsed = pd.to_numeric(cleaned, errors="coerce")
        if parsed.notna().mean() >= _NUMERIC_COERCION_THRESHOLD:
            # astype("string") (pandas' nullable string dtype), not astype(str)
            # (plain Python str) -- the latter would turn actual missing
            # values into the literal text "nan" before reparsing, which
            # risks corrupting null-handling for this column.
            cleaned_full = _clean_numeric_string_series(df[col].astype("string"))
