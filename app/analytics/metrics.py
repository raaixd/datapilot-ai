...
def compute_result_metrics(
    ...
    if len(values) == 0 and not has_dimension and not has_period:
        # A plain aggregate (SUM/AVG/MIN/MAX) over zero source rows returns
        # exactly one row containing SQL NULL -- not an error, but not a
        # real number either. Flagging this explicitly (rather than
        # silently reporting only row_count=1, which reads as "there IS an
        # answer") is what lets the insight narration say plainly that
        # there's no data to compute from, instead of a vague "returned 1
        # row(s)" that could be misread as a successful answer.
        metrics[metric_alias] = None
